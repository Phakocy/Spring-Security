...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        CustomAuthenticationFilter customAuthenticationFilter = new CustomAuthenticationFilter(authenticationManagerBean()); // Use this object to change the URL
        customAuthenticationFilter.setFilterProcessesUrl("/api/login"); // change to api/login instead of using ordinary login in the address
        http.csrf().disable();
        http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
        // http.authorizeRequests().anyRequest().permitAll(); EVERYBODY HAS ACCESS INSTEAD DO
           http.authorizeRequests().antMatchers("/api/login/**", "/api/token/refresh/**").permitAll(); //pass in only endpoints that you don't want to secure like "/login" spring have catered for it in UsernamePasswordAuthenticationFilter
